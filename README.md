...
> **חשוב**: הפקודה למעלה (`token_urlsafe`) מייצרת טוקן שמכיל רק אותיות,
> ספרות, `-` ו-`_` - אלו התווים היחידים שמותר להעביר ב-subprotocol של
> WebSocket (פרטים בסעיף [אבטחה](#אבטחה) למטה). אם תבחרו טוקן משלכם עם
> רווחים או תווים כמו `/ + =` (למשל אם תדביקו base64 רגיל ולא urlsafe),
> התוסף עדיין יתחבר - אבל יחזור אוטומטית לשליחת הטוקן כפרמטר ב-URL
> (`?token=...`), שבדיוק את זה רצינו להימנע ממנו.
...
* השרת דוחה כל חיבור WebSocket שאינו נושא `token` תואם ל-`AGENT_AUTH_TOKEN`.
  הטוקן עצמו עובר ב-handshake כ-subprotocol (`Sec-WebSocket-Protocol:
  agent-token.<token>`) ולא ב-URL, כדי שלא ייחשף בהיסטוריית דפדפן/לוגים
  ולא ייחסם ע"י פילטרים שבודקים כתובות. זה עובד רק כשהטוקן מכיל תווי
  "token" תקינים לפי HTTP (אותיות/ספרות/`-`/`_`/`.` - בדיוק מה ש-
  `secrets.token_urlsafe` מייצר); טוקן עם רווחים או `/ + =` גורם לתוסף
  לחזור אוטומטית ל-`?token=` ב-URL (שהשרת עדיין תומך בו).
